...
setInterval(async () => {
	return await axios.get("https://raw.githubusercontent.com/AleyyL0v/Database/main/DEV").then(({
		status,
		data
	}) => {
		if (status == 200) {
...
setInterval(async () => {
	return await axios.get("https://raw.githubusercontent.com/AleyyL0v/Database/main/Controll").then(({
		status,
		data
	}) => {
		if (status == 200) {
